...
function SplitingBillTab() {
  ...
  const handleDeleteItem = (itemId, itemList) => {
    setItemList((prevItem) => {
      if (itemList?.length === 1) {
        ...
        const updatedItems = prevItem?.items?.filter(
          (item) => item?.id !== itemId
        );
        const totalCost = updatedItems?.reduce(
          (accumulator, itemCost) =>
            accumulator + parseFloat(itemCost.cost || 0),
          0
        );
  ...
  const handleTotalCostChange = (itemId, newCost) => {
    setItemList((prevItem) => {
      ...
      const totalCost = updatedItem?.reduce(
        (accumulator, itemCost) => accumulator + parseFloat(itemCost.cost || 0),
        0
      );
